...
void AssignPointsToBoxes(struct Point *pointset, struct Box *box, int*j);
...
int main(int argc, char *argv[]){
    ...
    if (rank==0){          //Initial work is done by first process
      ...
      int counterC=0;
      for (int i=0; i<volume; i++){
        AssignPointsToBoxes(q, &boxessQ[i], &counterQ);
        AssignPointsToBoxes(c, &boxessC[i], &counterC);
      ...
      printf("Time to create points and arrange boxes :%f\n", seq_time1);
      gettimeofday (&startwtime, NULL);
...
outbox to the other processes */
      ...
      printf("Process %d, changed %d out of %d points\n", rank, f, d);
      ...
      for (int i=0; i<volume/processNumber; i++){
        for (int k=0; k<countBoxesQ[i]; k++){
          ...
          boxesQ[i].points[k].closestNeighbourId=possibleCandidate.id;
          boxesQ[i].points[k].distance= Distance(&possibleCandidate, &boxesQ[i].points[k]);
        }
      }
      ...
      for (int i=0; i<processNumber; i++){
        ...
        printf("Process %d received %d points from %d \n",rank , elements,i);
        for (int j=0; j<elements; j++){
          int a=WhichBox(inboxOne[i][j].neighbourBoxId, boxesQ);
      ...
      for (int i=0; i<processNumber; i++){
        int b=CountElements(inboxTwo[i]);
        for (int j=0; j<b; j++){
          f=f+CheckPoint(&inboxTwo[i][j], boxesQ);
        }
        d=d+b;
      }
      printf("Process %d, changed %d out of %d points\n", rank, f, d);
...
void AssignPointsToBoxes(struct Point *pointset, struct Box *box, int *j){
  int counter=0;
  for (int i=*j; i<numberOfPoints; i++){
...
/*Returns local number of process*/
...
/*Converts box coordinates to box id */
...
/* Creates a map which contains all neighbour boxes id*/
...
/* Sends initial boxes to processes (blocking) */
...
/*Receives initial boxes (blocking)*/
...
void ReceiveBoxes(struct Box *boxes, int rank, int *counter){
  for (int i=0; i<volume/processNumber; i++){
    ...
    incomingBuffer=malloc(10*numberOfPoints*sizeof(struct Point));
    MPI_Recv(incomingBuffer, 10*numberOfPoints, point_type, 0, 0, MPI_COMM_WORLD, &status);
...
/* Creates MPI type for struct Point */
...
/* For point given, search its box for closest neighbour*/
...
/* For a point given, find where it belongs*/
...
/* Check if a point is close to the edge of its box */
...
/* Send points to the proper processes */
...
/* Check if point received from other process, is closer than the closest neighbour
i found in the local box. */
